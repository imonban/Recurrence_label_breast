...
'''
directory = 'E:/IWE/Koo_project/Oncoshare_v3/'
def encoding(note):
...
df3 = pd.read_csv(directory + '/' +'STANFORD_NOTE_DATA_TABLE.csv', encoding = 'latin1')
...
## radiology
df1 = pd.read_csv(directory + '/' + "STANFORD_RADIOLOGY_DATA_TABLE.csv", encoding='latin1')
#df1 = df1[df1['ANON_ID'].isin(ids)].reset_index(drop = True)
df1 = report_read(df1)
df1['NOTE_TYPE'] = 'RAD_REPORT'
print ('Number of radiology note' + str(df1.shape))
## pathology
df2 = pd.read_csv(directory + '/' + "STANFORD_PATHOLOGY_DATA_TABLE.csv", encoding='latin1')
...
pat_df.to_csv('E:/IWE/Koo_project/Code/outcome/Patient_encounters.csv')
...
qaurter_wise_prediction.to_csv('E:/IWE/Koo_project/Code/outcome/quarters.csv')
...
pred.to_excel('E:/IWE/Koo_project/Code/outcome/Oncosharev3_CNN.xlsx')

pred  = classification_dense(qaurter_wise_prediction)
pred['START_DATE'] =  pd.to_datetime(pred['START_DATE'])
pred['END_DATE'] =  pd.to_datetime(pred['END_DATE'])
pred.to_excel('E:/IWE/Koo_project/Code/outcome/Oncosharev3_dense.xlsx')
